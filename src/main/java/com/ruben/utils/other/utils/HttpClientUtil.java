...
public class HttpClientUtil {
     ...
     * 2.自定义信任管理器       使用
     */
    public String sendHttpPost(String url, Map<String, String> maps, String xmlParam, boolean isHttps) throws UnsupportedEncodingException {
        //封装HttpPost
        ...
        if (maps != null) {
            ...
            httpPost.setEntity(new UrlEncodedFormEntity(nvps, Consts.CHARSET_UTF8)); // 设置参数
        }
        //2.Sting类型xml数据 如:wxpay
        if (xmlParam != null) {
            httpPost.setEntity(new StringEntity(xmlParam, Consts.CHARSET_UTF8));     // String的xml直接当尾参数传递进来 ★★
        }
        ...
        try {
            ...
            try {
                if (response != null) {
                    ...
                    // 响应内容
                    content = EntityUtils.toString(entity, Consts.CHARSET_UTF8);
                }
            } finally {
                response.close();
            }
