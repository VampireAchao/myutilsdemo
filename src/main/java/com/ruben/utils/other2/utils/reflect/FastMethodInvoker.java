package com.ruben.utils.other2.utils.reflect;
...
public class FastMethodInvoker {
	...
	public static FastMethodInvoker create(final Class<?> clz, final String methodName, Class<?>... parameterTypes) {
		Method method = com.jeeplus.common.utils.reflect.ClassUtil.getAccessibleMethod(clz, methodName, parameterTypes);
		if (method == null) {
	...
	public static FastMethodInvoker createGetter(final Class<?> clz, final String propertyName) {
		Method method = com.jeeplus.common.utils.reflect.ClassUtil.getGetterMethod(clz, propertyName);
		if (method == null) {
	...
	public static FastMethodInvoker createSetter(final Class<?> clz, final String propertyName,
                                                 Class<?> parameterType) {
		Method method = com.jeeplus.common.utils.reflect.ClassUtil.getSetterMethod(clz, propertyName, parameterType);
		if (method == null) {
			throw new IllegalArgumentException(
					"Could not find getter method [" + propertyName + "] on target [" + clz + ']');
		}
